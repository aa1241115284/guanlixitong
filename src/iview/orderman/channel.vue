...
            </el-option>
          ...
          <el-form-item label="广告公司名称" prop="channelName">
            <el-select
...
import { advertisingOrderlist, advertisingOrderdetails, advertisingOrderoperation, advertisingMap, membergetModules, ossupload } from '@/api/api'
export default {
  data () {
    return {
      btndetails: btnjust('订单管理', '广告渠道开通订单', 'onestop:advertisingOrder:details'),
      submList: [],
      err_ruleForm: {
        failReason: ''
      },
  ...
  methods: {
    ...
    init () {
      advertisingOrderlist(this.searchData).then((res) => {
        // console.log(res)
    ...
    // 广告公司名称
    getChannelMapinit () {
      advertisingMap(this.searchData).then((res) => {
        if (res.data.data) {
    ...
    err_submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        ...
        advertisingOrderoperation(obj).then((res) => {
          if (res.data.code == '0') {
    ...
    packClick (row) {
      ...
      advertisingOrderdetails({ orderId: row.orderId }).then((res) => {
        if (res.data.code == '0') {
