...
            </el-option>
          ...
          <el-form-item label="公司注册国家" prop='channelName'>
            <el-select
          ...
          >
            ...
            >
              <el-option
                :label="items"
                :value="items"
                v-for="items in functionList(item.configValue)"
              ></el-option>
            </el-select>
          </el-form-item>
...
import { companyorderlist, companyorderdetails, companyoperation, configgetChannelMap, membergetModules, ossupload } from '@/api/api'
export default {
  data () {
    return {
      btndetails: btnjust('订单管理', '公司注册订单', 'onestop:companyOrder:details'),
      submList: [],
      err_ruleForm: {
        failReason: ''
      },
  ...
  methods: {
    ...
    init () {
      companyorderlist(this.searchData).then((res) => {
        // console.log(res)
    ...
    // 公司网红名称
    getChannelMapinit () {
      configgetChannelMap(this.searchData).then((res) => {
    ...
    packClick (row) {
      ...
      companyorderdetails({ orderId: row.orderId }).then((res) => {
        if (res.data.code == '0') {
