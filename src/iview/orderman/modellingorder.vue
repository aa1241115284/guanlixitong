...
import { websitelist, websitedetails, websiteoperation, getChannelMap, membergetModules, ossupload } from '@/api/api'
export default {
  data () {
    return {
      btndetails: btnjust('订单管理', '建站申请订单', 'onestop:websiteOrder:details'),
      submList: [],
      err_ruleForm: {
        failReason: ''
      },
  ...
  methods: {
    ...
    init () {
      websitelist(this.searchData).then((res) => {
        // console.log(res)
    ...
    packClick (row) {
      ...
      websitedetails({ orderId: row.orderId }).then((res) => {
        if (res.data.code == '0') {
