...
import { celebrityOrderlist, celebrityOrderdetails, celeoperation, configgetChannelMap, membergetModules, ossupload } from '@/api/api'
export default {
  data () {
    return {
      btndetails: btnjust('订单管理', '网红营销订单', 'onestop:celebrityOrder:details'),
      submList: [],
      err_ruleForm: {
        failReason: ''
      },
  ...
  methods: {
    ...
    init () {
      celebrityOrderlist(this.searchData).then((res) => {
        // console.log(res)
    ...
    packClick (row) {
      ...
      celebrityOrderdetails({ orderId: row.orderId }).then((res) => {
        if (res.data.code == '0') {
