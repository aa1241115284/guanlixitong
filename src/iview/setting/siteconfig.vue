  ...
  <div class="body">
    <div class="concent">
      <div class="v-search">
        ...
        >
          <el-form-item label="运营商名称" prop="channelName">
            <el-input
      ...
      <Table border :columns="columns1" :data="data1" style="margin-top: 10px">
        ...
        <template slot-scope="{ row, index }" slot="action">
            ...
            <Button type="error" size="small" @click="personnel('冻结',false,row.templateId)" v-if='row.enable==true && btnupdateStatus' >冻结</Button >
             <Button type="success" size="small" @click="personnel('开启',true,row.templateId)" v-if='row.enable==false && btnupdateStatus' >开启</Button >
    ...
    <el-dialog :title="title" :visible.sync="applymodel" width="1000px">
      <el-form
        ...
        class="demo-ruleForm"
      >
        <el-form-item label="建站运营商名称" prop="channelName">
          <el-input
        ...
        <el-form-item label="建站运营商简介" prop="channelIntroduce">
          <el-input
            type="textarea"
            placeholder="请输入内容"
        ...
        <div class="teshu_class">
          <el-form-item
            label="建站运营商LOGO:  (请上传100*100,大小不超过1M的png/jpg的图片)"
            prop="channelLogo"
          >
            <el-upload
...
import { websiteconfiglist, websiteStatus, websiteaddPayment, websiteconfigedit, websiteconfigupdate, ossupload } from '@/api/api'
...
export default {
  ...
  data () {
    return {
      btnupdateStatus: btnjust('设置', '建站参数配置', 'onestop:websiteConfig:updateStatus'),
      btnadd: btnjust('设置', '建站参数配置', 'onestop:websiteConfig:add'),
      btnupdate: btnjust('设置', '建站参数配置', 'onestop:websiteConfig:update'),
      ruleForm: {
        channelName: '',
        channelIntroduce: '',
        channelLogo: '',
      ...
      type: '',
      rules: {
        channelName: [{ required: true, message: '请输入运营商名称' }],
        channelIntroduce: [{ required: true, message: '请输入运营商简介' }],
        channelLogo: [{ required: true, message: '请上传运营商LOGO' }]
      },
      title: '支付方添加',
      applymodel: false,
      loan_list_size: 1,
  ...
  methods: {
    init () {
      websiteconfiglist(this.searchData).then((res) => {
        console.log(res)
    ...
    submClick (formName) {
      this.$refs[formName].validate((valid) => {
        ...
        if (this.type == 'add') {
          websiteaddPayment(this.ruleForm).then((res) => {
            console.log(res)
            if (res.data.code == '0') {
          ...
          websiteconfigupdate(this.ruleForm).then((res) => {
            if (res.data.code == '0') {
              this.applymodel = false
              this.init()
            } else {
    ...
    operationclick (type, templateId) {
      ...
      if (type == 'add') {
        ...
        this.title = '建站运营商编辑'
        websiteconfigedit({templateId: templateId}).then((res) => {
          console.log(res)
    ...
    personnel (text, type, templateId) {
      this.$confirm(`是否${text}`, '提示', {
        ...
        .then(() => {
          websiteStatus({templateId: templateId, enable: type}).then((res) => {
            this.init()
          })
        })
        .catch(() => {})
