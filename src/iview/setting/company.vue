  ...
  <div class="body">
    <div class="concent">
      <div class="v-search">
        ...
        >
          <el-form-item label="国家名称" prop="channelName">
            <el-input
    ...
    <el-dialog :title="title" :visible.sync="applymodel" width="1000px">
      <el-form
        ...
        class="demo-ruleForm"
      >
        <el-form-item label="公司注册国名称" prop="channelName">
          <el-input
        ...
        <el-form-item label="公司注册国简介" prop="channelIntroduce">
          <el-input
            type="textarea"
            placeholder="请输入内容"
        ...
        <div class="teshu_class">
          <el-form-item
            label="公司注册所属国国旗:  (请上传100*100,大小不超过1M的png/jpg的图片)"
            prop="channelLogo"
          >
            <el-upload
...
import { companylist, companyupdateStatus, companyaddCompany, companyedit, companyconfigupdate, ossupload } from '@/api/api'
...
export default {
  ...
  data () {
    return {
      btnupdateStatus: btnjust('设置', '公司注册参数配置', 'onestop:companyConfig:updateStatus'),
      btnadd: btnjust('设置', '公司注册参数配置', 'onestop:companyConfig:add'),
      btnupdate: btnjust('设置', '公司注册参数配置', 'onestop:companyConfig:update'),
      ruleForm: {
        channelName: '',
        channelIntroduce: '',
        channelLogo: '',
      ...
      type: '',
      rules: {
        channelName: [{ required: true, message: '请输入国家名称' }],
  ...
  methods: {
    init () {
      companylist(this.searchData).then((res) => {
        console.log(res)
    ...
    submClick (formName) {
      this.$refs[formName].validate((valid) => {
        ...
        if (this.type == 'add') {
          companyaddCompany(this.ruleForm).then((res) => {
            console.log(res)
            if (res.data.code == '0') {
          ...
          companyconfigupdate(this.ruleForm).then((res) => {
            if (res.data.code == '0') {
              this.applymodel = false
              this.init()
            } else {
    ...
    operationclick (type, templateId) {
      ...
      if (type == 'add') {
        ...
        this.title = '公司注册国家编辑'
        companyedit({templateId: templateId}).then((res) => {
          console.log(res)
    ...
    personnel (text, type, templateId) {
      this.$confirm(`是否${text}`, '提示', {
        ...
        .then(() => {
          companyupdateStatus({templateId: templateId, templateStatus: type}).then((res) => {
            this.init()
          })
        })
        .catch(() => {})
