  ...
  <div class="body">
    <div class="concent">
      <div class="v-search">
        ...
        >
          <el-form-item label="支付方名称" prop="channelName">
            <el-input
    ...
    <el-dialog :title="title" :visible.sync="applymodel" width="1000px">
      ...
      >
        ...
        <el-form-item label="支付方简介" prop="channelIntroduce">
          <el-input
            type="textarea"
            placeholder="请输入内容"
        ...
        <div class="teshu_class">
          <el-form-item
            label="支付LOGO:  (请上传100*100,大小不超过1M的png/jpg的图片)"
            prop="channelLogo"
          >
            <el-upload
...
import { configlist, updateStatus, addPayment, configedit, configupdate, ossupload } from '@/api/api'
...
export default {
  ...
  data () {
    return {
      btnupdateStatus: btnjust('设置', '支付参数配置', 'onestop:paymentConfig:updateStatus'),
      btnadd: btnjust('设置', '支付参数配置', 'onestop:paymentConfig:add'),
      btnupdate: btnjust('设置', '支付参数配置', 'onestop:paymentConfig:update'),
      ruleForm: {
        channelName: '',
        channelIntroduce: '',
        channelLogo: '',
      ...
      type:"",
      rules: {
        channelName: [{ required: true, message: '请输入支付方名称' }],
  ...
  methods: {
    init () {
      configlist(this.searchData).then((res) => {
        console.log(res)
    ...
    submClick (formName) {
      this.$refs[formName].validate((valid) => {
        ...
        if(this.type=='add'){
        addPayment(this.ruleForm).then((res) => {
          console.log(res)
          if(res.data.code=='0'){
        ...
        configupdate(this.ruleForm).then((res)=>{
            if(res.data.code=='0'){
              this.applymodel=false
              this.init()
            }else{
    ...
    operationclick (type, templateId) {
      ...
      if (type == 'add') {
        ...
        this.title = '三方支付编辑'
        configedit({templateId: templateId}).then((res) => {
         console.log(res)
    ...
    personnel (text, type, templateId) {
      this.$confirm(`是否${text}`, '提示', {
        ...
        .then(() => {
          updateStatus({templateId: templateId, templateStatus: type}).then((res) => {
            this.init()
          })
        })
        .catch(() => {})
