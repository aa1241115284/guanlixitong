  ...
  <div class="body">
    <div class="concent">
      <div class="v-search">
        ...
        >
          <el-form-item label="广告渠道名称" prop="channelName">
            <el-input
    ...
    <el-dialog :title="title" :visible.sync="applymodel" width="1000px">
      <el-form
        ...
        class="demo-ruleForm"
      >
        <el-form-item label="广告渠道商名称" prop="channelName">
          <el-input
        ...
        <el-form-item label="广告渠道商简介" prop="channelIntroduce">
          <el-input
            type="textarea"
            placeholder="请输入内容"
        ...
        <div class="teshu_class">
          <el-form-item
            label="广告渠道商LOGO:  (请上传100*100,大小不超过1M的png/jpg的图片)"
            prop="channelLogo"
          >
            <el-upload
...
import { advertisinglist, advertisingStatus, addAdvertising, advertisingedit, advertisingupdage, ossupload } from '@/api/api'
...
export default {
  ...
  data () {
    return {
      btnupdateStatus: btnjust('设置', '广告参数配置', 'onestop:advertisingConfig:updateStatus'),
      btnadd: btnjust('设置', '广告参数配置', 'onestop:advertisingConfig:add'),
      btnupdate: btnjust('设置', '广告参数配置', 'onestop:advertisingConfig:update'),
      ruleForm: {
        channelName: '',
        channelIntroduce: '',
        channelLogo: '',
      ...
      type: '',
      rules: {
        channelName: [{ required: true, message: '请输入广告渠道名称' }],
        channelIntroduce: [{ required: true, message: '请输入支付方简介介' }],
        channelLogo: [{ required: true, message: '请上传支付LOGO' }]
      },
      title: '支付方添加',
      applymodel: false,
      loan_list_size: 1,
  ...
  methods: {
    init () {
      advertisinglist(this.searchData).then((res) => {
        console.log(res)
    ...
    submClick (formName) {
      this.$refs[formName].validate((valid) => {
        ...
        if (this.type == 'add') {
          addAdvertising(this.ruleForm).then((res) => {
            console.log(res)
            if (res.data.code == '0') {
          ...
          advertisingupdage(this.ruleForm).then((res) => {
            if (res.data.code == '0') {
              this.applymodel = false
              this.init()
            } else {
    ...
    operationclick (type, templateId) {
      ...
      if (type == 'add') {
        ...
        this.title = '广告渠道商编辑'
        advertisingedit({templateId: templateId}).then((res) => {
          console.log(res)
    ...
    personnel (text, type, templateId) {
      this.$confirm(`是否${text}`, '提示', {
        ...
        .then(() => {
          advertisingStatus({templateId: templateId, templateStatus: type}).then((res) => {
            this.init()
          })
        })
        .catch(() => {})
